...
def calculate_RSI(ticker, period=14):
    ...
    try:
        ...
        if RSI_rounded >= overbought_level:
            status = "Overbought"
        elif RSI_rounded <= oversold_level:
            status = "Oversold"
        else:
            status = "Neutral"

        return f"{RSI_rounded} - {status}"
...
def calculate_moving_average(ticker, period=50):
    if not isinstance(period, int):
        raise ValueError("Period must be an int.")
    
    stock = yf.Ticker(ticker)
    stock_data = stock.history(period=f"{period * 3}d") 

    if 'Close' not in stock_data or stock_data.empty:
        print(f"No data found for ticker {ticker}")
        return None
    
    MA = stock_data['Close'].rolling(window=period).mean()
    return round(MA.iloc[-1], 2) # rounded to 2 decimals
